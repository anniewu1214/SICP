...
(define (install-polynomial-package)
  ...
  (define (the-empty-termlist term-list)
    (let ((proc (get 'the-empty-termlist (type-tag term-list))))
      (if proc
          proc
          (error "THE-EMPTY-TERMLIST" term-list))))
  ...
  (define (adjoin-term term term-list)
    (let ((proc (get 'adjoin-term (type-tag term-list))))
      (if proc
          (proc term term-list)
          (error "No adjoin-term for this type" (list term term-list)))))
        ...
        (let ((t1 (first-term L1)) (t2 (first-term L2)))
          (if (> (order t2) (order t1))
              ...
              (let ((new-c (div (coeff t1) (coeff t2)))
                ...
                (let ((new-t (cons (type-tag L1) (make-term new-o new-c))))
                  (let ((rest-of-result (div-terms
                                         (sub-terms L1 (mul-terms new-t L2))
                                         L2)))
...
(define (install-sparse-term-package)
  (define (rest-terms term-list) (cons 'sparse (cddr term-list)))
  (define the-empty-termlist '(sparse))
  (define (empty-termlist? x) (equal? x the-empty-termlist))
  (define (make-poly variable terms) (make-poly variable (cons 'sparse terms)))
  ...
  (put 'the-empty-term-list 'sparse the-empty-termlist)
  (put 'make 'sparse make-poly)
  'done)

(define (install-dense-term-package)
  (define (rest-terms term-list) (cons 'dense (cddr term-list)))
  (define the-empty-termlist '(dense))
  (define (empty-termlist? x) (equal? x the-empty-termlist))
  (define (make-poly variable terms) (make-poly variable (cons 'dense terms)))
  
  (define (first-term term-list)
    (list (car term-list) (- (length term-list))))
  ...
  (put 'the-empty-term-list 'dense the-empty-termlist)
  (put 'make 'dense make-poly)
  'done)